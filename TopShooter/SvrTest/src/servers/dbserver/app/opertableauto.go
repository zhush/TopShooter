...
func Remove_t_account(key string)(bool, int){    redisKey := t_account+":"+key
	isExsit, _ := client.Exists(redisKey)
	if isExsit == false {
		return false, 0
	}
...
//删除表记录的方法，传入的是key 字符串,如果删除成功，则返回true,和影响的行数, 否则返回false, 0
func Remove_t_role(key string)(bool, int){    redisKey := t_role+":"+key
	isExsit, _ := client.Exists(redisKey)
	if isExsit == false {
		return false, 0
	}
...
//添加表记录的方法，传入的是json字符串,如果插入成功，则返回true,和自增的id, 否则返回false, 0
func Update_t_testtable(key string, contentJson string)(bool, int){
	var contentMaps map[string]interface{}
	err := json.Unmarshal(contentJson, &contentMaps)
	if err == nil {
		return false, 0
...
//删除表记录的方法，传入的是key 字符串,如果删除成功，则返回true,和影响的行数, 否则返回false, 0
func Remove_t_testtable(key string)(bool, int){    redisKey := t_testtable+":"+key
	isExsit, _ := client.Exists(redisKey)
	if isExsit == false {
		return false, 0
	}
