...
func (app *Application) Run() {
	log.Debug("HallServer is starting...")
	//启动自己的rpc服务
	app.serverName = config.Conf["Name"].(string)
	app.startRpcService()
	log.Debug("HallServer is Running!!")
	//连接管理器
...
func (app *Application) startRpcService() {
	log.Debug("HallServer is start rpc service...")
...
func (app *Application) Close() {
	log.Debug("HallServer is closed")
}
...
func (app *Application) RegisterToManager() {
	...
	if isOk == false {
		log.Error("Register HallServer(%s) to ManagerServer(%s) Failed!!", config.Conf["Addr"], config.Conf["ManagerAddr"])
		return
	}
	log.Debug("Register HallServer(%s) to ManagerServer(%s) Succeed!!", config.Conf["Addr"], config.Conf["ManagerAddr"])

	isOk1, _, _ := app.managerServer.SendMsg("BroadMessageToAllGame", "Hello, World, This is from GameServer:"+app.serverName)
	if isOk1 == false {
		log.Error("BroadToManager HallServer(%s) to ManagerServer(%s) Failed!!", config.Conf["Addr"], config.Conf["ManagerAddr"])
		return
	}
	log.Debug("BroadToManager HallServer(%s) to ManagerServer(%s) Succeed!!", config.Conf["Addr"], config.Conf["ManagerAddr"])
